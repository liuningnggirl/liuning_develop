...
//点击预约时间
		...
		$('.restimeselect .right').append('<option selected>请选择预约时段</option>');	
		...
		function yueFunc(e) {
			...
			checkAccessTokenLogin(function(){
				...
				if($('.restimeselect .right option:selected').html() == '' || $('.restimeselect .right option:selected').html() == '请选择预约时段'){
					alert('请选择预约时段');
				}else if($('#address').val() == ''){
					alert('请输入详细地址');
			...
			},'wantOrder-xinhe.html'+window.location.search);
		}
