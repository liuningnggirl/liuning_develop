$(function(){
	...
	//获取全国城市列表V2.4.2
	$.get('<%= UGC_HOST_API_URL %>/nggirl/app/allCommonCity/2.4.2',getFinalRequestObject({accessToken: getAccessToken()}),function(data){
	...
	$('.tba_ok').click(function(e) {
        ...
        $('.sheng li').each(function(index, element) {
            if($(this).hasClass('current_sheng')){
				sheng = $(this).html();
				$('.bc_address i').attr('provinceId',$(this).attr('provinceId'));
				$('.bc_address i').attr('sheng',$(this).html());
			}
        });
        $('.shi li').each(function(index, element) {
            if($(this).hasClass('current_shi')){
				...
				$('.box_area').hide();
				$('.bc_address i').attr('cityId',$(this).attr('cityId'));
				$('.bc_address i').attr('shi',$(this).html());
				$('.bc_address i').attr('cityCode',$(this).attr('cityCode'));
				if(shengshi != sheng+' '+shi && shengshi !='请选择地址'){
					$('.select-street .select-href').html('请选择').css('color','#9a9a9a');
				}else{
					
				}
				$('.bc_address i').html(shi).css('color','#3a3a3a');
			}
        });
    });
