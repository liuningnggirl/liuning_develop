...
$(function() {
	...
	//加载页面数据
	$.ajax({
				url : '/nggirl/app/activity/cosmcourse/getcourseinfo',
				type : 'get',
				dataType : 'json',
				...
				success : function(data) {
					...
					$('.classtypeselect .right').html(
							data.data.coursetypes[0].courseTypeName);

					if (data.data.reservationStatus == 1) {
						$('.yue-man').hide();
						$('.btn-yue .btn-ok').show();
					}
					if (data.data.reservationStatus == 2) {
						$('.btn-yue .btn-ok').attr('disabled', 'disabled');
						$('.btn-yue .btn-ok').css('background', '#A2A2A2');
						$('.btn-yue .btn-ok').show();
						$('.yue-man').show();
						var payTime = data.data.resservationWaitSeconds;
						var payMinite = Math.floor(payTime / 60);
						var paySecond = payTime % 60;
						if (payMinite == 0) {
							$('.btn-yue .btn-ok').val(
									'当前有人正在支付(请耐心等待' + paySecond + '秒)');
						}
						if (paySecond == 0) {
							$('.btn-yue .btn-ok').val(
									'当前有人正在支付(请耐心等待' + payMinite + '分)');
						}
						if (payMinite != 0 && paySecond != 0) {
							$('.btn-yue .btn-ok').val(
									'当前有人正在支付(请耐心等待' + payMinite + '分'
											+ paySecond + '秒)');
						}
					}
					if (data.data.reservationStatus == 3) {
						$('.yue-man').show();
						$('.btn-yue .btn-ok').show();
						$('.btn-yue .btn-ok').attr('disabled', 'disabled');
						$('.btn-yue .btn-ok').css('background', '#A2A2A2');
						$('.btn-yue .btn-ok').val(' 已约满');
					}
					if (data.data.reservationStatus == 4) {
						$('.yue-man').show();
						$('.btn-yue .btn-ok').show();
						$('.btn-yue .btn-ok').attr('disabled', 'disabled');
						$('.btn-yue .btn-ok').val('您已成功预约');
						$('.btn-yue .btn-ok').css('background', '#A2A2A2');
					}
				}
			});
	...
	$('.hzs .btn-ok').click(
			function(e) {
				...
				if (/iphone|ipad|ipod/.test(ua)) {
					_czc.push(['_trackEvent','jiaoxue','phoneType=iOS','click','order-btn']);   
				} else if (/android/.test(ua)) {
					 _czc.push(['_trackEvent','jiaoxue','phoneType=and','click','order-btn']);
				};
				...
				$.ajax({
					type : 'GET',
					url : '/nggirl/app/cli/checkAccessToken',
					data : data,
