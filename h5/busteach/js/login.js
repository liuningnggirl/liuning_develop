		$(function(){
			...
			$('.login-btn').click(function(e){
				if (/iphone|ipad|ipod/.test(ua)) {
					_czc.push(['_trackEvent','yue-btn','phoneType=iOS','qyjiaoxue','']);	

				} else if (/android/.test(ua)) {
					_czc.push(['_trackEvent','yue-btn','phoneType=and','qyjiaoxue','']);
				};	
				...
				$.ajax({
					type:'POST',
					url:'<%= CLI_HOST_API_URL %>/nggirl/app/cli/phoneUser/logon',
					data:data,
	...
	//忘记密码2-->确定
			$('.reset-pwd .btn-ok').click(function(e){
				if (/iphone|ipad|ipod/.test(ua)) {
					_czc.push(['_trackEvent','qyjiaoxue','phoneType=iOS','btn-reset-ps','']);	
				} else if (/android/.test(ua)) {
					_czc.push(['_trackEvent','qyjiaoxue','phoneType=and','btn-reset-ps','']);
				};
	...
	//注册2-->立即注册
			$('.register-success .btn-ok').click(function(e){
				...
				if (/iphone|ipad|ipod/.test(ua)) {
					_czc.push(['_trackEvent','qyjiaoxue','phoneType=iOS','btn-rigist']);   
				} else if (/android/.test(ua)) {
					 _czc.push(['_trackEvent','qyjiaoxue','phoneType=and','btn-rigist']);
				};
