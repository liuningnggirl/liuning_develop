	...
	$(function(){
		...
		var remoteurl ='<%= CLI_HOST_API_URL %>/nggirl/app/activity/cosmcourse/getcourseinfo';
		// 从地址栏中取得
...
//选择预约人数
	...
	function openBirthDate() {
		...
		for(var i=0;i<hours.length;i++){
			if((defaultTimeValue + hours[i]) == $('.Wdate').val()){
				defaultTimeIndex = i;
				break;
			}
		}
	...
	function reservationAndPay(){
		...
		if (/iphone|ipad|ipod/.test(ua)) {
			_czc.push(['_trackEvent','yue-btn','phoneType=iOS','pay-btn','']);	
		} else if (/android/.test(ua)) {
			_czc.push(['_trackEvent','yue-btn','phoneType=and','pay-btn','']);
		};	
		
		var coursetypeId = $('body').data('currentCoursetypeId');

		var resurl = '<%= CLI_HOST_API_URL %>/nggirl/app/activity/cosmcourse/ordercourse';
		var payurl = '<%= CLI_HOST_API_URL %>/nggirl/app/activity/cosmcourse/payCourseReservation';
		if($('#resultnum').html() == '请选择预约人数'){
			alert('请选择预约人数');
		}else if($('.Wdate').val() == '' || $('.Wdate').val() == '请选择预约时间'){
			alert('请选择预约时间');
		}else if($('#address').val() == ''){
			alert('请输入详细地址');	
