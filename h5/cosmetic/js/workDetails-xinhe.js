...
//点击“立即预约”按钮
    $('.f-center').click(function (e) {
        ...
        checkAccessTokenLogin(function(){
			...
			if(isInWeixin()){
				window.location.href = getWeixinLinkUrl(getParam('workId'),$('.banner').attr('dresserId'),$('.ot-data option:selected').html());
			}else{
				window.location.href = getZhifuBaoLinkUrl(getParam('workId'),$('.banner').attr('dresserId'),$('.ot-data option:selected').html());
			}
        },'workDetails-xinhe.html'+window.location.search);
    });
...
function getZhifuBaoLinkUrl(workId,dresserId,time){
    var str = "<%= WEIXIN_BOUND_DOMAIN_URL %>/nggirl/h5/cosmetic/wantOrder-xinhe.html?workId="
