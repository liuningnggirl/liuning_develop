...
//点击查看更多晒单
    ...
    $('.gl-pic>ul>li img').live("click", function (e) {
        window.location.href = "workDetails?workId=" + $(this).attr('workId')+'&v=<%= VERSION %>';
    });
...
//点击“立即预约”按钮
    $('.f-center').click(function (e) {
        ...
        checkAccessTokenLogin(function(){
            ...
            localStorage.setItem('time', '2016-06-01');
            localStorage.setItem('dresserId', $('.banner').attr('dresserId'));
			var time = $('.ot-data').val();
           
			if(isInWeixin()){
				window.location.href = getWeixinLinkUrl(getParam('workId'),$('.banner').attr('dresserId'),'2016-06-15');
			}else{
				window.location.href = getZhifuBaoLinkUrl(getParam('workId'),$('.banner').attr('dresserId'),'2016-06-15');
			}
        },'workDetails-cuc.html'+window.location.search);
...
function getZhifuBaoLinkUrl(workId,dresserId,time){
    var str = "<%= WEIXIN_BOUND_DOMAIN_URL %>/nggirl/h5/cosmetic/wantOrder-cuc.html?workId="
