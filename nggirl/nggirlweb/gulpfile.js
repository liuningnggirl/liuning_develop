...
gulp.task('clean', function () {
    ...
    for (var i = 0; i < config.source.jsFiles.length; i++) {
        var val = config.source.jsFiles[i];
        targetArr.push(val.replace('h5/', config.dest.packageDir))
    }
    for (var i = 0; i < config.source.cssFiles.length; i++) {
        var val = config.source.cssFiles[i];
        targetArr.push(val.replace('h5/', config.dest.packageDir))
    }
    for (var i = 0; i < config.source.imageFiles.length; i++) {
        var val = config.source.imageFiles[i];
        targetArr.push(val.replace('h5/', config.dest.packageDir))
    }
    for (var i = 0; i < config.source.htmlFiles.length; i++) {
        var val = config.source.htmlFiles[i];
        targetArr.push(val.replace('h5/', config.dest.packageDir))
    }
    gutil.log(targetArr);
    return gulp.src(targetArr, {read: false}).pipe(clean({force: true}));
});
...
gulp.task('pack-shtml', function () {
    return packShtml(config.source.specialHtmlFiles, config.dest.packageDir);
});
...
function packShtml(sourcePackFiles, destPackDir) {
    gutil.log('将shtml中的css/js/image路径替换');
    var sourceFiles = [config.dest.rootDir + '**/*.json'].concat(sourcePackFiles);
    gutil.log(sourceFiles);
    return gulp.src(sourceFiles)
        .pipe(template(config))
