...
/*保存某个商品属性*/
	$(".i_want_newcrease").live('click',function(){
				...
				$(this).parent().prev().append('<input type="text" name=""><img src="./images/remove_one.png" class="i_want_remove_prev" style="display:inline-block">')
	...
	$('.i_want_to_idit').live('click',function(){
		...
		if($(this).html()=='编辑'){
			...
			if(!$(this).attr("keyid")==""){
				$.post("<%= CLI_HOST_API_URL %>/nggirl-web/web/admin/item/saveProperty/3.1.0",{keyId:keyId,name:name,values:input_Arr_Value},function(data){
				var data=$.parseJSON(data)
					if(data.code==0){
						alert("保存成功")
					}
				...
				$.post("<%= CLI_HOST_API_URL %>/nggirl-web/web/admin/item/saveProperty/3.1.0",{name:name_ineed,values:input_Arr_Value},function(data){
					var data=$.parseJSON(data)
					if(data.code==0){
						alert("保存成功")
						wefhwihfeu()
