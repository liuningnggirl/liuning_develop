$(function(){
	...
	//热门话题-刷新
	$('.community_hottopic_manage .community_hottopic_manage_content .chmc_btn_refresh').click(function(e) {
		var btn = $(this);
	...
	$('.community_hottopic_manage_content .chmc_btn_add').click(function(e) {
		...
		$('.add_hottopic .add_hottopic_fengmian').removeAttr('src');
	...
	$('.add_hottopic .add_hottopic_cancle').click(function(e) {
		$('.community_hottopic_manage_content,#community_hottopic_manage').show();
		$('.add_hottopic').hide().removeAttr('topicId');
		$('.add_hottopic').removeAttr('initFansNum');
		$('.add_hottopic').removeAttr('initPostNum');
		$('.add_hottopic').removeAttr('recommendPosts');
	...
	$('.chmc_hot_sort .community_hottopic_table .cht_edit_btn').live('click',function(e) {
		...
		$.get('<%= CLI_HOST_API_URL %>/nggirl-web/web/admin/community/hottopic/getHotTopic/3.0.0',{topicId:btn.attr('topicId')},function(data){
			...
			if(data.code == 0){
				...
				$('.add_hottopic .add_hottopic_fengmian').attr('src',data.data.cover);
	...
	//添加话题封面图
	$('.add_hottopic .add_hottopic_fengmian').click(function(e) {
        $('.add_hottopic #add_hottopic_input_fengmian').click();
    });	
	$('.add_hottopic #add_hottopic_input_fengmian').fileupload({
		dataType: 'json',
		done: function (e, data) {
			var w = parseFloat(data.result.data.width);
			var h = parseFloat(data.result.data.height);
			if(w/h != 3/2){
				alert("上传失败！图片比例应为3*2");
			}else{
				$('.add_hottopic .add_hottopic_fengmian').attr('src',data.result.data.url);
			};
		}
	});	
