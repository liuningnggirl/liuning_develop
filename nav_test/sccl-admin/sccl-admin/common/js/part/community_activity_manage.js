$(function(){
	...
	$('.community_activity_manage_a .cfmc_content .cc_tools .cc_modify').live('click',function(e) {
		var btn = $(this);
		$.get('<%= CLI_HOST_API_URL %>/nggirl-web/web/admin/community/post/getRecommendInfo/3.0.0',{postId:btn.parent().prev().prev().prev().val(),postType:btn.parent().prev().children('option:selected').attr('value')},function(data){
	...
	$('.community_activity_manage_a .community_freetrail_manage_content .cfmc_btn_save').click(function(e) {
		...
		//posts = '['+posts.substring(0,posts.length-1)+']';
		console.log(posts);
		$.post('<%= CLI_HOST_API_URL %>/nggirl-web/web/admin/community/post/saveRecommendList/3.0.0',{posts:str},function(data){
			var data = $.parseJSON(data);
			if(data.code == 0){
...
function getRecommendListTuanFn(){
	...
	$.get('<%= CLI_HOST_API_URL %>/nggirl-web/web/admin/community/post/getRecommendList/3.0.0',function(data){
		var data = $.parseJSON(data);
		if(data.code == 0){
			if(data.data.length < 3 && data.data.length !=0){
				for(var x = 0; x < data.data.length; x ++){
					if(data.data[x].postType == 1){
					$('.community_activity_manage_a .community_freetrail_manage_content .cfmc_box').append('<div class="cfmc_content"><input type="text" class="cc_txt" disabled="disabled" value="'+data.data[x].title+'" /><label for=""> ID: </label><input type="text" class="cc_id" value="'+data.data[x].postId+'" disabled="disabled" /><label for=""> 帖子类型: </label><select class="cc_type" style="padding: 9px;border-radius: 5px;    border: 1px solid #ccc;" disabled="disabled"><option value="">请选择帖子类型</option><option value="1" selected="selected">文章</option><option value="2">视频</option></select><span class="cc_tools" style="display:none;"><img src="images/img_xiugai.png" class="cc_modify" postType='+data.data[x].postType+' postId='+data.data[x].postId+' alt="" /><img src="images/img_arr_down.png" class="cc_arr_down" alt="" /><img src="images/img_arr_up.png" class="cc_arr_up" alt="" /></span></div>');
					}else if(data.data[x].postType == 2){
					$('.community_activity_manage_a .community_freetrail_manage_content .cfmc_box').append('<div class="cfmc_content"><input type="text" class="cc_txt" disabled="disabled" value="'+data.data[x].title+'" /><label for=""> ID: </label><input type="text" class="cc_id" value="'+data.data[x].postId+'" disabled="disabled" /><label for=""> 帖子类型: </label><select class="cc_type" style="padding: 9px;border-radius: 5px;    border: 1px solid #ccc;" disabled="disabled"><option value="">请选择帖子类型</option><option value="1">文章</option><option value="2" selected="selected">视频</option></select><span class="cc_tools" style="display:none;"><img src="images/img_xiugai.png" class="cc_modify" postType='+data.data[x].postType+' postId='+data.data[x].postId+' alt="" /><img src="images/img_arr_down.png" class="cc_arr_down" alt="" /><img src="images/img_arr_up.png" class="cc_arr_up" alt="" /></span></div>');
					}
				}
				for(var x = 0; x < (3- data.data.length); x ++){
					$('.community_activity_manage_a .community_freetrail_manage_content .cfmc_box').append('<div class="cfmc_content"><input type="text" class="cc_txt" disabled="disabled" value="" /><label for=""> ID: </label><input type="text" class="cc_id" value="" disabled="disabled" /><label for=""> 帖子类型: </label><select class="cc_type" style="padding: 9px;border-radius: 5px; border: 1px solid #ccc;" disabled="disabled"><option value="">请选择帖子类型</option><option value="1">文章</option><option value="2">视频</option></select><span class="cc_tools" style="display:none;"><img src="images/img_xiugai.png" class="cc_modify" alt="" /><img src="images/img_arr_down.png" class="cc_arr_down" alt="" /><img src="images/img_arr_up.png" class="cc_arr_up" alt="" /></span></div>');
				}
