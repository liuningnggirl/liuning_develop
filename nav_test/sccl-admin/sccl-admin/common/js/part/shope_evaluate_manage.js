...
$(function(){
	...
	$(".store_information_now").live('click',function(){
		...
		$.each($(".choice_goods_picture_more_last_choicePic .div_three_common_oh"),function(key,val){
			var imgsUrl1= new Object;
			var chn=$(this).children(".needEditImg");
			imgsUrl1.url=chn.attr('src');
			imgsUrl1.width=chn.attr('width');
			imgsUrl1.height=chn.attr('height');
			headImg.push(imgsUrl1);
			return headImg;
		});
		var headImgs = JSON.stringify(headImg);
		...
		var newcre_categoryid_new=$(".lujing_gooods_choice").attr("categoryId");
		var isSupportPumpkinCoin = 0;
		if($("#judge_goods_shopping_five").attr("checked")){
			isSupportPumpkinCoin=1
...
//种草帖子跳转指定页面按钮
	...
	/*点击商品分类获取多级列表*/
	/*编辑商品点击商品分类获取多级列表*/
	$(".liuji_caidan_one .b").live("click",function(){
		if($(this).attr("index") != undefined){
			$(".choice_goods_category_ul_important").empty().append(allDetItemCategories($(this).attr("id")));
		}else{
			var a = $(this).parent().index();
			
			$(".choice_goods_category_ul_important").children('.liuji_caidan_one:gt('+a+')').remove();
			$(".choice_goods_category_ul_important").append(allDetItemCategories($(this).attr("id")));
		}
	});
	$(".liuji_caidan_one .c").live("click",function(){
		var a = $(this).parent().index();
		$(".choice_goods_category_ul_important").children('.liuji_caidan_one:gt('+a+')').remove();
		$(".lujing_gooods_choice").html($(this).text()).attr("categoryId",$(this).attr("id"));	
	});
	
	$(".goods_management_focus_click .b").live("click",function(){
		var index = $(this).parent().index()-1;
		$(this).parent().parent().children('.goods_management_focus_click_chilComn:gt('+index+')').empty();
		$(this).parent().next().append(allItemCategories($(this).attr("id")));
	});
	$(".goods_management_focus_click .c").live("click",function(){
		var index = $(this).parent().index()-1;
		$(this).parent().parent().children('div.goods_management_focus_click_chilComn:gt('+index+')').empty();
		$(".input_focus_click").val($(this).text()).attr("categoryId",$(this).attr("id"));	
	});
		...
		$.get(testUrl+"/nggirl-web/web/admin/item/getItemCategories/3.1.0",function(data){
			...
			for(var i=0;i<data.data.length;i++){
				if(data.data[i].isLeaf==0){
					 $('.goods_management_focus_click').data('a'+data.data[i].id,data.data[i].children);
					 str+='<li class="b " id="'+data.data[i].id+'" index="'+i+'">'+data.data[i].name+'</li>'
				}else{
					 str+='<li class="c" id="'+data.data[i].id+'" index="'+i+'">'+data.data[i].name+'</li>'
				}
			}
			$(".goods_management_focus_click_chilComn_one").html(str);
			$(".goods_management_focus_click").css("display",'block');
			$('div.goods_management_focus_click_chilComn:gt(0)').empty();
		});
	})
	/*点击确定按钮*/
...
$(".zengjia_pointer").live('click',function(){
		...
		$.get(testUrl+"/nggirl-web/web/admin/item/getItemCategories/3.1.0",function(data){
			...
			for(var i=0;i<data.data.length;i++){
				if(data.data[i].isLeaf==0){
					 $('.choice_goods_category').data('a'+data.data[i].id,data.data[i].children);
					 str+='<li class="b " id="'+data.data[i].id+'" index="'+i+'">'+data.data[i].name+'</li>'
				}else{
					 str+='<li class="c" id="'+data.data[i].id+'" index="'+i+'">'+data.data[i].name+'</li>'
				}
			...
			$('.choice_goods_category_ul_important').empty();
		});
		/*新增销售属性信息*/
		...
		$(".lujing_gooods_choice,.goods_newCrease_name_span").html("")
		...
		$('.input_select_value').removeAttr("id");
		$('.lujing_gooods_choice').removeAttr("categoryId");
		$('.goods_shaling_information_sku_chilr').empty();
		$('.shangpin_wuliu_information_getinformation').removeAttr("checked");
		$(".addOrEditShop").html("&gt;编辑商品");
		if($(this).val()=="新增商品"){
			shopGoodEdit();
			$("#judge_goods_shopping_one").attr("checked","checked")
			$("#judge_goods_shopping_three").attr("checked","checked")
			$("#judge_goods_shopping_five").attr("checked","checked")
			$(".judge_goods_shopping_days").val('7')
...
/*------------------------------商品sku-----------------------------------*/
			...
			$(".lujing_gooods_choice").attr("categoryId",data.data.categoryId)
			...
			if(data.data.isSupportPumpkinCoin==0){
				$("#judge_goods_shopping_six").attr("checked","checked")
			}else if(data.data.isSupportPumpkinCoin==1){
				$("#judge_goods_shopping_five").attr("checked","checked")
			}
			$(".judge_goods_shopping_days").val(data.data.serviceTerm)
...
function getShopParams(page){
	...
	params.categoryId = $.trim($('.electricity_supplier_goods_manage .search_input_fillin .categoryId').attr("categoryId"));
...
function allItemCategories(num){
	var data = $('.goods_management_focus_click').data('a'+num);
	var str='';
	for(var i=0;i<data.length;i++){
		if(data[i].isLeaf==0){
			 $('.goods_management_focus_click').data('a'+data[i].id,data[i].children);
			 str+='<li class="b " id="'+data[i].id+'" index="'+num+i+'">'+data[i].name+'</li>'
		}else{
			 str+='<li class="c" id="'+data[i].id+'" index="'+num+i+'">'+data[i].name+'</li>'
		}
	};
	return str;
}
function allDetItemCategories(num){
	var data = $('.choice_goods_category').data('a'+num);
	var str='';
	str +='<div class="liuji_caidan_one">';
	for(var i=0;i<data.length;i++){
		if(data[i].isLeaf==0){
			 $('.choice_goods_category').data('a'+data[i].id,data[i].children);
			 str+='<li class="b " id="'+data[i].id+'">'+data[i].name+'</li>'
		}else{
			 str+='<li class="c" id="'+data[i].id+'" >'+data[i].name+'</li>'
		}
	};
	str +='</div>';
	return str;
