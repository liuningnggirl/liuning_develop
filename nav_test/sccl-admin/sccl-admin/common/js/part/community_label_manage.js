...
$(function(){
	...
	$(".label_manage_box .labelTypeDetail .addNewLabel").live("click",function(){
		var str='';
		str +='<div class="labelBox"><input type="text" value="" placeholder="标签" class="labelone"><img src="../common/images/ele-del.png" class="delLabel" alt="" /></div>';
		$(this).before(str);
	});
	
	$(".label_manage_box .labelTypeDetail .addNewTopicLabel").live("click",function(){
		var str='';
		str +='<div class="labelBox">'+getAllHotTopicDetail()+'<img src="../common/images/ele-del.png" class="delLabel" alt="" /></div>';
		$(this).before(str);
		$(".labelTypeBox .label_name").removeAttr("disabled");
	});
...
function getLabelList(){
	$.ajax({
		url : testUrl+'/nggirl-web/web/admin/community/label/getAllLables/4.0.0',
		type : 'get',
		dataType : 'json',
		data: {},
				...
				$.each(data.data,function(key,val){
					...
					if(key ==0){
						$.each(val.labels,function(key,val1){
							str +='<div class="labelBox"><input type="text" value="'+val1.name+'" class="labelone"><img src="../common/images/ele-del.png" class="delLabel" alt="" /></div>';	
						});
						str +='<input type="button" value="添加" class="addNewLabel"></div></div>';
					}else{
						$.each(val.labels,function(key,val1){
							str +='<div class="labelBox">'+getAllHotTopicDetail(val1.topicId)+'<img src="../common/images/ele-del.png" class="delLabel" alt="" /></div>';	
						});
...
function saveLabelList(){
	...
	if(canSave != 2){
		$.ajax({
			url : testUrl+'/nggirl-web/web/admin/community/label/saveAllLables/4.0.0',
			type : 'post',
			dataType : 'json',
...
function getAllHotTopic(){
	$.get(testUrl+'/nggirl-web/web/admin/column/getAllHotTopic/2.5.3',function(data){
