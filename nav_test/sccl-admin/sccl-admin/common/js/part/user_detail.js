...
$(function(){
	...
	$('.ucm_edit_user_message .ueum_ok_btn').click(function(e) {
		...
		$.post(testUrl+'/nggirl-web/web/admin/user/updateUseRole/3.0.0',{userId:$('.user_competence_manage .ucm_del_all_ping').attr('userId'),originalUserRoleId:originalUserRoleId,currentUserRoleId:$('.ucm_edit_user_message .ueum_text').val()},function(data){
			var data = $.parseJSON(data);
			if(data.code == 0){
	...
	$('.table_user_competence .tuc_jin_btn').live('click',function(e) {
		var r = confirm('确定要禁言？');
		if(r == true){
			$.post(testUrl+'/nggirl-web/web/admin/user/makeUserSilent/2.2.0',{userId:$('.user_competence_manage .ucm_del_all_ping').attr('userId')},function(data){
				var data = $.parseJSON(data);
				if(data.code == 0){
					getUserManageMessageFn($('.user_competence_manage .ucm_del_all_ping'));
				}else{
	...
	$('.table_user_competence .tuc_jin_cancle_btn').live('click',function(e) {
		...
		if(r == true){
			$.post(testUrl+'/nggirl-web/web/admin/user/makeUserSpeak/2.2.0',{userId:$('.user_competence_manage .ucm_del_all_ping').attr('userId')},function(data){
				var data = $.parseJSON(data);
				if(data.code == 0){
					getUserManageMessageFn($('.user_competence_manage .ucm_del_all_ping'));
				}else{
	...
	$('.table_user_competence .tuc_feng_btn').live('click',function(e) {
		var r = confirm('确定要封号？');
		if(r == true){
			$.post(testUrl+'/nggirl-web/web/admin/user/forbidUserAccount/2.2.0',{userId:$('.user_competence_manage .ucm_del_all_ping').attr('userId')},function(data){
				var data = $.parseJSON(data);
				if(data.code == 0){
					getUserManageMessageFn($('.user_competence_manage .ucm_del_all_ping'));
				}else{
	...
	$('.table_user_competence .tuc_feng_cancle_btn').live('click',function(e) {
		...
		if(r == true){
			$.post(testUrl+'/nggirl-web/web/admin/user/openUserAccount/2.2.0',{userId:$('.user_competence_manage .ucm_del_all_ping').attr('userId')},function(data){
				var data = $.parseJSON(data);
				if(data.code == 0){
					getUserManageMessageFn($('.user_competence_manage .ucm_del_all_ping'));
				}else{
	...
	$('.user_another_window .user_another_btn_ok').click(function(e) {
		$.post(testUrl+'/nggirl-web/web/admin/user/userGetAdditionalScore/2.5.6',{userId:$('.user_competence_manage .ucm_del_all_ping').attr('userId'),score:$('.user_another_window .user_another_window_txt').val()},function(data){
			var data = $.parseJSON(data);
			if(data.code == 0){
...
function createUserPage(data){
	$(".yhxqq .tcdPageCode").createPage({
		...
		backFn:function(p){
			...
			$.ajax({
				url : testUrl+'/nggirl-web/web/admin/user/listUsers/2.5.6',
				type : 'get',
				dataType : 'json',
				data: params,
...
function getUserManageMessageFn(btn){
	$.get(testUrl+'/nggirl-web/web/admin/user/getUserPrivilegeInfo/2.2.0',{userId:btn.attr('userId')},function(data){
		var data = $.parseJSON(data);
		if(data.code == 0){
...
function loadUserPointsStatistics(){
	$.ajax({
		url : testUrl+'/nggirl-web/web/admin/stat/userScoreStat/3.0.0',
		type : 'get',
		dataType : 'json',
