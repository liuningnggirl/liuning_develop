...
$(function(){	
	...
	$('.table-message.shope_order_manage_table .look_details').live('click',function(e) {
		var btn = $(this);
		$.get(testUrl+'/nggirl-web/web/admin/item/order/getOrderDetail/3.1.0',{orderId:btn.parent().parent().attr('orderId')},function(data){
	...
	$('.shope_order_manage_look_order_message .shope_order_manage_look_order_message_save_btn').click(function(e) {
		...
		if(r == true){
			$.post(testUrl+'/nggirl-web/web/admin/item/order/saveOrder/3.1.0',paramData,function(data){
				var data = $.parseJSON(data);
				if(data.code == 0){
	...
	$('.shope_order_manage_table .reimburse').live('click',function(e) {
		...
		var btn = $(this);
		$.get(testUrl+'/nggirl-web/web/admin/item/order/getRefundOrder/3.1.0',{orderId:btn.parent().parent().attr('orderId')},function(data){
			var data = $.parseJSON(data);
			if(data.code == 0){
	...
	$('.shope_order_manage_tuikuan_content_reason #check_open').click(function(e) {
		...
		if(r == true){
			$.get(testUrl+'/nggirl-web/web/admin/item/order/confirmRefund/3.1.0',{orderId:$('.shope_order_manage_tuikuan_content').attr('orderId')},function(data){
				var data = $.parseJSON(data);
				if(data.code == 0){
	...
	$('.shope_order_manage_business_note .save_btn').click(function(e) {
        if($.trim($('.shope_order_manage_business_note textarea').val()) == ''){
			...
			$.post(testUrl+'/nggirl-web/web/admin/item/order/sellerNote/3.1.0',{orderId:$('.shope_order_manage_business_note').attr('orderId'),descrip:$.trim($('.shope_order_manage_business_note textarea').val())},function(data){
				var data = $.parseJSON(data);
				if(data.code == 0){
	...
	$('.shope_order_manage_tuikuan_content .un_pass .send_btn').click(function(e) {
		...
		if($.trim($('.shope_order_manage_business_note textarea').val()) == ''){
			...
			$.post(testUrl+'/nggirl-web/web/admin/item/order/confirmNoRefund/3.1.0',{orderId:$('.shope_order_manage_tuikuan_content').attr('orderId'),descrip:$.trim($('.shope_order_manage_business_note textarea').val())},function(data){
				var data = $.parseJSON(data);
				if(data.code == 0){
					$('.shope_order_manage_business_note').hide();
					loadShopeGetOrderPage();
	...
	$('.shope_order_manage_tuikuan_content .pass .sure_btn').click(function(e) {
		...
		if(r == true){
			if($.trim($('.shope_order_manage_tuikuan_content .pass .tuikuan_money').val()) == ''){
				...
				$.post(testUrl+'/nggirl-web/web/admin/item/order/refund/3.1.0',{orderId:$('.shope_order_manage_tuikuan_content').attr('orderId'),refundAmount:$.trim($('.shope_order_manage_tuikuan_content .pass .tuikuan_money').val())},function(data){
					var data = $.parseJSON(data);
					if(data.code == 0){
	...
	$('#shope_order_manage .shope_order_manage_table .shope_order_manage_table_modify_money').live('click',function(e) {
		...
		if(r == true){
			if($.trim(btn.prev()) == ''){
				...
				$.post(testUrl+'/nggirl-web/web/admin/item/order/modifyOrder/3.1.0',{orderId:btn.parent().parent().attr('orderId'),price:$.trim(btn.prev().attr('value'))},function(data){
					var data = $.parseJSON(data);
					if(data.code == 0){
						alert('修改成功！！');
					}else{
	...
	$('#shope_order_manage .shope_order_manage_table .shope_order_manage_table_modify_status').live('click',function(e) {
		...
		if(r == true){
			$.get(testUrl+'/nggirl-web/web/admin/item/order/changeOrderStatus/3.1.0',{orderId:btn.parent().parent().attr('orderId'),orderStatusId:btn.prev().children('option:selected').attr('value')},function(data){
				var data = $.parseJSON(data);
				if(data.code == 0){
					alert('修改成功！！');
					loadShopeGetOrderPage();
	...
	$('#shope_order_manage .shope_order_manage_table .modify_wuliu_hao').live('click',function(e) {
		...
		if(r == true){
			$.post(testUrl+'/nggirl-web/web/admin/item/order/modifyOrderTrans/3.1.0',{orderId:btn.parent().parent().attr('orderId'),expressNum:$.trim(btn.prev().val()),expressCode:btn.prev().prev().children('option:selected').attr('code')},function(data){
				var data = $.parseJSON(data);
				if(data.code == 0){
					alert('修改成功！！');
				}else{
	...
	$('#shope_order_manage .shope_order_manage_table .close_order').live('click',function(e) {
		...
		if(r == true){
			$.get(testUrl+'/nggirl-web/web/admin/item/order/closeOrder/3.1.0',{orderId:btn.parent().parent().attr('orderId')},function(data){
				var data = $.parseJSON(data);
				if(data.code == 0){
					loadShopeGetOrderPage();
				}else{
	...
	$('.shope_order_manage .shope_order_manage_content_li .shope_order_manage_content_li_daochu').click(function(e) {
		...
		if(orderIds == '[]'){
			...
			window.location.href = testUrl+"/nggirl-web/web/admin/item/order/exportOrderToExcel/3.1.0?orderIds=" + orderIds+'&v=<%= VERSION %>';
		}
    });
	...
	$('#shope_order_manage .shope_order_manage_table .look_wuliu').live('click',function(e) {
		...
		$.get(testUrl+'/nggirl-web/web/admin/item/service/getExpressTrace/3.1.0',{expressNum:$.trim(btn.parent().children('.text_padding').val()),companyCode:btn.parent().children('.wuliu_shang').children('option:selected').attr('code')},function(data){
			var data = $.parseJSON(data);
			if(data.code == 0){
	...
	$('#shope_order_manage .shope_order_manage_content_li .shope_order_manage_content_li_del').click(function(e) {
		...
		if(orderIds != '[]'){
			...
			if(r == true){
				$.post(testUrl+'/nggirl-web/web/admin/item/order/deleteOrder/3.1.0',{orderIds:orderIds},function(data){
					var data = $.parseJSON(data);
					if(data.code == 0){
						loadShopeGetOrderPage();
					}else{
	...
	$('.shope_order_manage .shope_order_manage_content_li .shope_order_manage_content_li_suoding').click(function(e) {
		...
		if(orderIds != '[]'){
			...
			if(r == true){
				$.post(testUrl+'/nggirl-web/web/admin/item/order/lockOrder/3.1.0',{orderIds:orderIds},function(data){
					var data = $.parseJSON(data);
					if(data.code == 0){
						loadShopeGetOrderPage();
					}else{
	...
	$('.shope_order_manage .shope_order_manage_content_li .shope_order_manage_content_li_shifang').click(function(e) {
		...
		if(orderIds != '[]'){
			...
			if(r == true){
				$.post(testUrl+'/nggirl-web/web/admin/item/order/unlockOrder/3.1.0',{orderIds:orderIds},function(data){
					var data = $.parseJSON(data);
					if(data.code == 0){
						loadShopeGetOrderPage();
					}else{
	...
	$('.shope_order_manage_content_li .shope_order_manage_content_li_fahuo').click(function(e) {
		...
		if(orderIds != '[]'){
			var r = confirm('确认要发货？？');
			if(r == true){
				$.post(testUrl+'/nggirl-web/web/admin/item/order/getSendList/3.1.0',{orderIds:orderIds},function(data){
					var data = $.parseJSON(data);
					if(data.code == 0){
	...
	$('.shope_order_manage_fahuo .fahuo_btn').click(function(e) {
		...
		if(r == true){
			if(sendSelectGoods == '[]'){
				...
				$.post(testUrl+'/nggirl-web/web/admin/item/order/sendGoods/3.1.0',{sendSelectGoods:sendSelectGoods},function(data){
					var data = $.parseJSON(data);
					if(data.code == 0){
						$('.shope_order_manage_fahuo .shope_order_manage_fahuo_table tbody tr').each(function(index, element) {
...
function createShopeGetOrderPage(data){
	$("#shope_order_manage .tcdPageCode").createPage({
		...
		backFn:function(p){
			...
			$.ajax({
				url : testUrl+'/nggirl-web/web/admin/item/order/getOrder/3.1.0',
				type : 'get',
				dataType : 'json',
				data: params,
	...
	if(sku.length > 0){
				...
				if(data.isRefund == 0){//未做过退款申请
					...
					if(data.canModify == 0){
						...
						if(data.orderStatus[0].num == 1){
							$('.shope_order_manage .shope_order_manage_table tbody').append('<tr orderId='+data.orderId+'><td rowspan='+sku.length+'><input type="checkbox" /><span>订单编号:'+data.orderCode+'</span><br /><span>支付来源：'+data.paySource+'</span></td><td><p>'+sku[x].title+'</p></td><td>'+sku[x].num+'</td><td rowspan='+sku.length+'>'+data.createTime+'</td><td rowspan='+sku.length+'><input type="text" value="'+data.payAcount+'" style=" width:50px; text-align:center;" class="text_padding" /><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_money" /></td><td>'+data.pumpkinCoin+'南瓜币/'+data.discountMoney+'元'+'</td><td rowspan='+sku.length+'><select name="" id="" style=" padding:7px; border:1px solid #ccc;">'+getWuLiuOrderStatusMessage(data.orderStatus)+'</select><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_status" /></td><td class="hidden" rowspan='+sku.length+'>'+data.reciveTime+'</td><td class="hidden" rowspan='+sku.length+'>'+data.applyRefundTime+'</td><td rowspan='+sku.length+'><select name="" id="" style="padding:7px; border:1px solid #ccc;" class="wuliu_shang">'+getWuLiuMessageFn(data.transCompanyName)+'</select><input type="text" value="'+data.transNum+'" class="text_padding" style="width:100px; padding: 8px;vertical-align: bottom;"/><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="modify_wuliu_hao" /><br /><br /><div class="look_wuliu"><img src="../common/images/look_icon.jpg" style="width:20px; vertical-align:bottom;" alt="" /><span>查看物流</span></div></td><td rowspan='+sku.length+'>'+data.customerNote+'</td><td rowspan='+sku.length+'>'+data.sellerNote+'</td><td rowspan='+sku.length+'>无</td><td rowspan='+sku.length+'>'+getMenuStatusFn(data.strings)+'</td><td rowspan='+sku.length+'>'+data.modifyer+'</td><td rowspan='+sku.length+'>'+data.customerName+'</td><td rowspan='+sku.length+'>'+data.customerPhone+'</td><td rowspan='+sku.length+'>'+data.customerAddress+'</td></tr>');
						}else{
						...
						//仅待付款状态可修改金额
						if(data.orderStatus[0].num == 1){
							$('.shope_order_manage .shope_order_manage_table tbody').append('<tr orderId='+data.orderId+'><td rowspan='+sku.length+'><input type="checkbox" /><span>订单编号:'+data.orderCode+'</span><br /><span>支付来源：'+data.paySource+'</span></td><td><p>'+sku[x].title+'</p></td><td>'+sku[x].num+'</td><td rowspan='+sku.length+'>'+data.createTime+'</td><td rowspan='+sku.length+'><input type="text" value="'+data.payAcount+'" style=" width:50px; text-align:center;" class="text_padding" /><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_money" /></td><td>'+data.pumpkinCoin+'南瓜币/'+data.discountMoney+'元'+'</td><td rowspan='+sku.length+'><select name="" id="" style=" padding:7px; border:1px solid #ccc;">'+getWuLiuOrderStatusMessage(data.orderStatus)+'</select><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_status" /></td><td class="hidden" rowspan='+sku.length+'>'+data.reciveTime+'</td><td class="hidden" rowspan='+sku.length+'>'+data.applyRefundTime+'</td><td rowspan='+sku.length+'>物流单号：'+data.transNum+'</td><td rowspan='+sku.length+'>'+data.customerNote+'</td><td rowspan='+sku.length+'>'+data.sellerNote+'</td><td rowspan='+sku.length+'>无</td><td rowspan='+sku.length+'>'+getMenuStatusFn(data.strings)+'</td><td rowspan='+sku.length+'>'+data.modifyer+'</td><td rowspan='+sku.length+'>'+data.customerName+'</td><td rowspan='+sku.length+'>'+data.customerPhone+'</td><td rowspan='+sku.length+'>'+data.customerAddress+'</td></tr>');
						}else{
							$('.shope_order_manage .shope_order_manage_table tbody').append('<tr orderId='+data.orderId+'><td rowspan='+sku.length+'><input type="checkbox" /><span>订单编号:'+data.orderCode+'</span><br /><span>支付来源：'+data.paySource+'</span></td><td><p>'+sku[x].title+'</p></td><td>'+sku[x].num+'</td><td rowspan='+sku.length+'>'+data.createTime+'</td><td rowspan='+sku.length+'>'+data.payAcount+'</td><td>'+data.pumpkinCoin+'南瓜币/'+data.discountMoney+'元'+'</td><td rowspan='+sku.length+'><select name="" id="" style=" padding:7px; border:1px solid #ccc;">'+getWuLiuOrderStatusMessage(data.orderStatus)+'</select><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_status" /></td><td class="hidden" rowspan='+sku.length+'>'+data.reciveTime+'</td><td class="hidden" rowspan='+sku.length+'>'+data.applyRefundTime+'</td><td rowspan='+sku.length+'>物流单号：'+data.transNum+'</td><td rowspan='+sku.length+'>'+data.customerNote+'</td><td rowspan='+sku.length+'>'+data.sellerNote+'</td><td rowspan='+sku.length+'>无</td><td rowspan='+sku.length+'>'+getMenuStatusFn(data.strings)+'</td><td rowspan='+sku.length+'>'+data.modifyer+'</td><td rowspan='+sku.length+'>'+data.customerName+'</td><td rowspan='+sku.length+'>'+data.customerPhone+'</td><td rowspan='+sku.length+'>'+data.customerAddress+'</td></tr>');
						}
					}
					...
					if(data.refundStatus == 0){//退款申请状态,[0-未处理,1-已处理]
						...
						if(data.canModify == 0){
							//仅待付款状态可修改金额
							if(data.orderStatus[0].num == 1){
								$('.shope_order_manage .shope_order_manage_table tbody').append('<tr orderId='+data.orderId+'><td rowspan='+sku.length+'><input type="checkbox" /><span>订单编号:'+data.orderCode+'</span><br /><span>支付来源：'+data.paySource+'</span></td><td><p>'+sku[x].title+'</p></td><td>'+sku[x].num+'</td><td rowspan='+sku.length+'>'+data.createTime+'</td><td rowspan='+sku.length+'><input type="text" value="'+data.payAcount+'" style=" width:50px; text-align:center;" class="text_padding" /><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_money" /></td><td>'+data.pumpkinCoin+'南瓜币/'+data.discountMoney+'元'+'</td><td rowspan='+sku.length+'><select name="" id="" style=" padding:7px; border:1px solid #ccc;">'+getWuLiuOrderStatusMessage(data.orderStatus)+'</select><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_status" /></td><td class="hidden" rowspan='+sku.length+'>'+data.reciveTime+'</td><td class="hidden" rowspan='+sku.length+'>'+data.applyRefundTime+'</td><td rowspan='+sku.length+'><select name="" id="" style="padding:7px; border:1px solid #ccc;" class="wuliu_shang">'+getWuLiuMessageFn(data.transCompanyName)+'</select><input type="text" value="'+data.transNum+'" class="text_padding" style="width:100px; padding: 8px;vertical-align: bottom;"/><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="modify_wuliu_hao" /><br /><br /><div class="look_wuliu"><img src="../common/images/look_icon.jpg" style="width:20px; vertical-align:bottom;" alt="" /><span>查看物流</span></div></td><td rowspan='+sku.length+'>'+data.customerNote+'</td><td rowspan='+sku.length+'>'+data.sellerNote+'</td><td rowspan='+sku.length+'>'+data.refundReason+'<p class="font_color_red">（未处理）</p></td><td rowspan='+sku.length+'>'+getMenuStatusFn(data.strings)+'</td><td rowspan='+sku.length+'>'+data.modifyer+'</td><td rowspan='+sku.length+'>'+data.customerName+'</td><td rowspan='+sku.length+'>'+data.customerPhone+'</td><td rowspan='+sku.length+'>'+data.customerAddress+'</td></tr>');
							}else{
								$('.shope_order_manage .shope_order_manage_table tbody').append('<tr orderId='+data.orderId+'><td rowspan='+sku.length+'><input type="checkbox" /><span>订单编号:'+data.orderCode+'</span><br /><span>支付来源：'+data.paySource+'</span></td><td><p>'+sku[x].title+'</p></td><td>'+sku[x].num+'</td><td rowspan='+sku.length+'>'+data.createTime+'</td><td rowspan='+sku.length+'>'+data.payAcount+'</td><td>'+data.pumpkinCoin+'南瓜币/'+data.discountMoney+'元'+'</td><td rowspan='+sku.length+'><select name="" id="" style=" padding:7px; border:1px solid #ccc;">'+getWuLiuOrderStatusMessage(data.orderStatus)+'</select><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_status" /></td><td class="hidden" rowspan='+sku.length+'>'+data.reciveTime+'</td><td class="hidden" rowspan='+sku.length+'>'+data.applyRefundTime+'</td><td rowspan='+sku.length+'><select name="" id="" style="padding:7px; border:1px solid #ccc;" class="wuliu_shang">'+getWuLiuMessageFn(data.transCompanyName)+'</select><input type="text" value="'+data.transNum+'" class="text_padding" style="width:100px; padding: 8px;vertical-align: bottom;"/><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="modify_wuliu_hao" /><br /><br /><div class="look_wuliu"><img src="../common/images/look_icon.jpg" style="width:20px; vertical-align:bottom;" alt="" /><span>查看物流</span></div></td><td rowspan='+sku.length+'>'+data.customerNote+'</td><td rowspan='+sku.length+'>'+data.sellerNote+'</td><td rowspan='+sku.length+'>'+data.refundReason+'<p class="font_color_red">（未处理）</p></td><td rowspan='+sku.length+'>'+getMenuStatusFn(data.strings)+'</td><td rowspan='+sku.length+'>'+data.modifyer+'</td><td rowspan='+sku.length+'>'+data.customerName+'</td><td rowspan='+sku.length+'>'+data.customerPhone+'</td><td rowspan='+sku.length+'>'+data.customerAddress+'</td></tr>');
							}
						}else{
							//仅待付款状态可修改金额
							if(data.orderStatus[0].num == 1){
								$('.shope_order_manage .shope_order_manage_table tbody').append('<tr orderId='+data.orderId+'><td rowspan='+sku.length+'><input type="checkbox" /><span>订单编号:'+data.orderCode+'</span><br /><span>支付来源：'+data.paySource+'</span></td><td><p>'+sku[x].title+'</p></td><td>'+sku[x].num+'</td><td rowspan='+sku.length+'>'+data.createTime+'</td><td rowspan='+sku.length+'><input type="text" value="'+data.payAcount+'" style=" width:50px; text-align:center;" class="text_padding" /><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_money" /></td><td>'+data.pumpkinCoin+'南瓜币/'+data.discountMoney+'元'+'</td><td rowspan='+sku.length+'><select name="" id="" style=" padding:7px; border:1px solid #ccc;">'+getWuLiuOrderStatusMessage(data.orderStatus)+'</select><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_status" /></td><td class="hidden" rowspan='+sku.length+'>'+data.reciveTime+'</td><td class="hidden" rowspan='+sku.length+'>'+data.applyRefundTime+'</td><td rowspan='+sku.length+'>物流单号：'+data.transNum+'</td><td rowspan='+sku.length+'>'+data.customerNote+'</td><td rowspan='+sku.length+'>'+data.sellerNote+'</td><td rowspan='+sku.length+'>'+data.refundReason+'<p class="font_color_red">（未处理）</p></td><td rowspan='+sku.length+'>'+getMenuStatusFn(data.strings)+'</td><td rowspan='+sku.length+'>'+data.modifyer+'</td><td rowspan='+sku.length+'>'+data.customerName+'</td><td rowspan='+sku.length+'>'+data.customerPhone+'</td><td rowspan='+sku.length+'>'+data.customerAddress+'</td></tr>');
							}else{
								$('.shope_order_manage .shope_order_manage_table tbody').append('<tr orderId='+data.orderId+'><td rowspan='+sku.length+'><input type="checkbox" /><span>订单编号:'+data.orderCode+'</span><br /><span>支付来源：'+data.paySource+'</span></td><td><p>'+sku[x].title+'</p></td><td>'+sku[x].num+'</td><td rowspan='+sku.length+'>'+data.createTime+'</td><td rowspan='+sku.length+'>'+data.payAcount+'</td><td>'+data.pumpkinCoin+'南瓜币/'+data.discountMoney+'元'+'</td><td rowspan='+sku.length+'><select name="" id="" style=" padding:7px; border:1px solid #ccc;">'+getWuLiuOrderStatusMessage(data.orderStatus)+'</select><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_status" /></td><td class="hidden" rowspan='+sku.length+'>'+data.reciveTime+'</td><td class="hidden" rowspan='+sku.length+'>'+data.applyRefundTime+'</td><td rowspan='+sku.length+'>物流单号：'+data.transNum+'</td><td rowspan='+sku.length+'>'+data.customerNote+'</td><td rowspan='+sku.length+'>'+data.sellerNote+'</td><td rowspan='+sku.length+'>'+data.refundReason+'<p class="font_color_red">（未处理）</p></td><td rowspan='+sku.length+'>'+getMenuStatusFn(data.strings)+'</td><td rowspan='+sku.length+'>'+data.modifyer+'</td><td rowspan='+sku.length+'>'+data.customerName+'</td><td rowspan='+sku.length+'>'+data.customerPhone+'</td><td rowspan='+sku.length+'>'+data.customerAddress+'</td></tr>');
							}
						}
						...
						if(data.canModify == 0){
							//仅待付款状态可修改金额
							if(data.orderStatus[0].num == 1){
								$('.shope_order_manage .shope_order_manage_table tbody').append('<tr orderId='+data.orderId+'><td rowspan='+sku.length+'><input type="checkbox" /><span>订单编号:'+data.orderCode+'</span><br /><span>支付来源：'+data.paySource+'</span></td><td><p>'+sku[x].title+'</p></td><td>'+sku[x].num+'</td><td rowspan='+sku.length+'>'+data.createTime+'</td><td rowspan='+sku.length+'><input type="text" value="'+data.payAcount+'" style=" width:50px; text-align:center;" class="text_padding" /><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_money" /></td><td>'+data.pumpkinCoin+'南瓜币/'+data.discountMoney+'元'+'</td><td rowspan='+sku.length+'><select name="" id="" style=" padding:7px; border:1px solid #ccc;">'+getWuLiuOrderStatusMessage(data.orderStatus)+'</select><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_status" /></td><td class="hidden" rowspan='+sku.length+'>'+data.reciveTime+'</td><td class="hidden" rowspan='+sku.length+'>'+data.applyRefundTime+'</td><td rowspan='+sku.length+'><select name="" id="" style="padding:7px; border:1px solid #ccc;" class="wuliu_shang">'+getWuLiuMessageFn(data.transCompanyName)+'</select><input type="text" value="'+data.transNum+'" class="text_padding" style="width:100px; padding: 8px;vertical-align: bottom;"/><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="modify_wuliu_hao" /><br /><br /><div class="look_wuliu"><img src="../common/images/look_icon.jpg" style="width:20px; vertical-align:bottom;" alt="" /><span>查看物流</span></div></td><td rowspan='+sku.length+'>'+data.customerNote+'</td><td rowspan='+sku.length+'>'+data.sellerNote+'</td><td rowspan='+sku.length+'>'+data.refundReason+'<p class="font_color_red">（已处理）</p></td><td rowspan='+sku.length+'>'+getMenuStatusFn(data.strings)+'</td><td rowspan='+sku.length+'>'+data.modifyer+'</td><td rowspan='+sku.length+'>'+data.customerName+'</td><td rowspan='+sku.length+'>'+data.customerPhone+'</td><td rowspan='+sku.length+'>'+data.customerAddress+'</td></tr>');
							}else{
								$('.shope_order_manage .shope_order_manage_table tbody').append('<tr orderId='+data.orderId+'><td rowspan='+sku.length+'><input type="checkbox" /><span>订单编号:'+data.orderCode+'</span><br /><span>支付来源：'+data.paySource+'</span></td><td><p>'+sku[x].title+'</p></td><td>'+sku[x].num+'</td><td rowspan='+sku.length+'>'+data.createTime+'</td><td rowspan='+sku.length+'>'+data.payAcount+'</td><td>'+data.pumpkinCoin+'南瓜币/'+data.discountMoney+'元'+'</td><td rowspan='+sku.length+'><select name="" id="" style=" padding:7px; border:1px solid #ccc;">'+getWuLiuOrderStatusMessage(data.orderStatus)+'</select><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_status" /></td><td class="hidden" rowspan='+sku.length+'>'+data.reciveTime+'</td><td class="hidden" rowspan='+sku.length+'>'+data.applyRefundTime+'</td><td rowspan='+sku.length+'><select name="" id="" style="padding:7px; border:1px solid #ccc;" class="wuliu_shang">'+getWuLiuMessageFn(data.transCompanyName)+'</select><input type="text" value="'+data.transNum+'" class="text_padding" style="width:100px; padding: 8px;vertical-align: bottom;"/><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="modify_wuliu_hao" /><br /><br /><div class="look_wuliu"><img src="../common/images/look_icon.jpg" style="width:20px; vertical-align:bottom;" alt="" /><span>查看物流</span></div></td><td rowspan='+sku.length+'>'+data.customerNote+'</td><td rowspan='+sku.length+'>'+data.sellerNote+'</td><td rowspan='+sku.length+'>'+data.refundReason+'<p class="font_color_red">（已处理）</p></td><td rowspan='+sku.length+'>'+getMenuStatusFn(data.strings)+'</td><td rowspan='+sku.length+'>'+data.modifyer+'</td><td rowspan='+sku.length+'>'+data.customerName+'</td><td rowspan='+sku.length+'>'+data.customerPhone+'</td><td rowspan='+sku.length+'>'+data.customerAddress+'</td></tr>');
							}
						}else{
							//仅待付款状态可修改金额
							if(data.orderStatus[0].num == 1){
								$('.shope_order_manage .shope_order_manage_table tbody').append('<tr orderId='+data.orderId+'><td rowspan='+sku.length+'><input type="checkbox" /><span>订单编号:'+data.orderCode+'</span><br /><span>支付来源：'+data.paySource+'</span></td><td><p>'+sku[x].title+'</p></td><td>'+sku[x].num+'</td><td rowspan='+sku.length+'>'+data.createTime+'</td><td rowspan='+sku.length+'><input type="text" value="'+data.payAcount+'" style=" width:50px; text-align:center;" class="text_padding" /><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_money" /></td><td>'+data.pumpkinCoin+'南瓜币/'+data.discountMoney+'元'+'</td><td rowspan='+sku.length+'><select name="" id="" style=" padding:7px; border:1px solid #ccc;">'+getWuLiuOrderStatusMessage(data.orderStatus)+'</select><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_status" /></td><td class="hidden" rowspan='+sku.length+'>'+data.reciveTime+'</td><td class="hidden" rowspan='+sku.length+'>'+data.applyRefundTime+'</td><td rowspan='+sku.length+'>物流单号：'+data.transNum+'</td><td rowspan='+sku.length+'>'+data.customerNote+'</td><td rowspan='+sku.length+'>'+data.sellerNote+'</td><td rowspan='+sku.length+'>'+data.refundReason+'<p class="font_color_red">（已处理）</p></td><td rowspan='+sku.length+'>'+getMenuStatusFn(data.strings)+'</td><td rowspan='+sku.length+'>'+data.modifyer+'</td><td rowspan='+sku.length+'>'+data.customerName+'</td><td rowspan='+sku.length+'>'+data.customerPhone+'</td><td rowspan='+sku.length+'>'+data.customerAddress+'</td></tr>');
							}else{
								$('.shope_order_manage .shope_order_manage_table tbody').append('<tr orderId='+data.orderId+'><td rowspan='+sku.length+'><input type="checkbox" /><span>订单编号:'+data.orderCode+'</span><br /><span>支付来源：'+data.paySource+'</span></td><td><p>'+sku[x].title+'</p></td><td>'+sku[x].num+'</td><td rowspan='+sku.length+'>'+data.createTime+'</td><td rowspan='+sku.length+'>'+data.payAcount+'</td><td>'+data.pumpkinCoin+'南瓜币/'+data.discountMoney+'元'+'</td><td rowspan='+sku.length+'><select name="" id="" style=" padding:7px; border:1px solid #ccc;">'+getWuLiuOrderStatusMessage(data.orderStatus)+'</select><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_status" /></td><td class="hidden" rowspan='+sku.length+'>'+data.reciveTime+'</td><td class="hidden" rowspan='+sku.length+'>'+data.applyRefundTime+'</td><td rowspan='+sku.length+'>物流单号：'+data.transNum+'</td><td rowspan='+sku.length+'>'+data.customerNote+'</td><td rowspan='+sku.length+'>'+data.sellerNote+'</td><td rowspan='+sku.length+'>'+data.refundReason+'<p class="font_color_red">（已处理）</p></td><td rowspan='+sku.length+'>'+getMenuStatusFn(data.strings)+'</td><td rowspan='+sku.length+'>'+data.modifyer+'</td><td rowspan='+sku.length+'>'+data.customerName+'</td><td rowspan='+sku.length+'>'+data.customerPhone+'</td><td rowspan='+sku.length+'>'+data.customerAddress+'</td></tr>');
							}
						}
		...
		if(data.isRefund == 0){//未做过退款申请
			...
			if(data.canModify == 0){
				//仅待付款状态可修改金额
				if(data.orderStatus[0].num == 1){
					$('.shope_order_manage .shope_order_manage_table tbody').append('<tr orderId='+data.orderId+'><td><input type="checkbox" /><span>订单编号:'+data.orderCode+'</span><br /><span>支付来源：'+data.paySource+'</span></td><td></td><td></td><td>'+data.createTime+'</td><td><input type="text" value="'+data.payAcount+'" style=" width:50px; text-align:center;" class="text_padding" /><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_money" /></td><td>'+data.pumpkinCoin+'南瓜币/'+data.discountMoney+'元'+'</td><td><select name="" id="" style=" padding:7px; border:1px solid #ccc;">'+getWuLiuOrderStatusMessage(data.orderStatus)+'</select><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_status" /></td><td class="hidden" rowspan='+sku.length+'>'+data.reciveTime+'</td><td class="hidden" rowspan='+sku.length+'>'+data.applyRefundTime+'</td><td><select name="" id="" style="padding:7px; border:1px solid #ccc;" class="wuliu_shang">'+getWuLiuMessageFn(data.transCompanyName)+'</select><input type="text" value="'+data.transNum+'" class="text_padding" style="width:100px; padding: 8px;vertical-align: bottom;"/><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="modify_wuliu_hao" /><br /><br /><div class="look_wuliu"><img src="../common/images/look_icon.jpg" style="width:20px; vertical-align:bottom;" alt="" /><span>查看物流</span></div></td><td>'+data.customerNote+'</td><td>'+data.sellerNote+'</td><td>无</td><td>'+getMenuStatusFn(data.strings)+'</td><td>'+data.modifyer+'</td><td>'+data.customerName+'</td><td>'+data.customerPhone+'</td><td>'+data.customerAddress+'</td></tr>');
				}else{
				...
				//仅待付款状态可修改金额
				if(data.orderStatus[0].num == 1){
					$('.shope_order_manage .shope_order_manage_table tbody').append('<tr orderId='+data.orderId+'><td><input type="checkbox" /><span>订单编号:'+data.orderCode+'</span><br /><span>支付来源：'+data.paySource+'</span></td><td></td><td></td><td>'+data.createTime+'</td><td><input type="text" value="'+data.payAcount+'" style=" width:50px; text-align:center;" class="text_padding" /><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_money" /></td><td>'+data.pumpkinCoin+'南瓜币/'+data.discountMoney+'元'+'</td><td><select name="" id="" style=" padding:7px; border:1px solid #ccc;">'+getWuLiuOrderStatusMessage(data.orderStatus)+'</select><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_status" /></td><td class="hidden" rowspan='+sku.length+'>'+data.reciveTime+'</td><td class="hidden" rowspan='+sku.length+'>'+data.applyRefundTime+'</td><td>物流单号：'+data.transNum+'</td><td>'+data.customerNote+'</td><td>'+data.sellerNote+'</td><td>无</td><td>'+getMenuStatusFn(data.strings)+'</td><td>'+data.modifyer+'</td><td>'+data.customerName+'</td><td>'+data.customerPhone+'</td><td>'+data.customerAddress+'</td></tr>');
				}else{
					$('.shope_order_manage .shope_order_manage_table tbody').append('<tr orderId='+data.orderId+'><td><input type="checkbox" /><span>订单编号:'+data.orderCode+'</span><br /><span>支付来源：'+data.paySource+'</span></td><td></td><td></td><td>'+data.createTime+'</td><td>'+data.payAcount+'</td><td>'+data.pumpkinCoin+'南瓜币/'+data.discountMoney+'元'+'</td><td><select name="" id="" style=" padding:7px; border:1px solid #ccc;">'+getWuLiuOrderStatusMessage(data.orderStatus)+'</select><img src="../common/images/u84.png" style="width:20px; vertical-align: middle;" alt="" class="shope_order_manage_table_modify_status" /></td><td class="hidden" rowspan='+sku.length+'>'+data.reciveTime+'</td><td class="hidden" rowspan='+sku.length+'>'+data.applyRefundTime+'</td><td>物流单号：'+data.transNum+'</td><td>'+data.customerNote+'</td><td>'+data.sellerNote+'</td><td>无</td><td>'+getMenuStatusFn(data.strings)+'</td><td>'+data.modifyer+'</td><td>'+data.customerName+'</td><td>'+data.customerPhone+'</td><td>'+data.customerAddress+'</td></tr>');
				}
			}
...
function getMenuStatusFn(menu){
	...
	for(var x = 0; x < menu.length; x ++){
		if(menu[x] == '退款管理'){
			str += '<div class="reimburse" style="text-align:left;"><img src="../common/images/tuikuan_icon.png" style="width:20px; vertical-align:bottom;" alt="" /> <span>退款管理</span></div>';
		}else if(menu[x] == '发货'){
			str += '<div class="ship" style="text-align:left;"><img src="../common/images/huoche_icon.png" style="width:20px; vertical-align:bottom;" alt="" /> <span>发货</span></div>';
		}else if(menu[x] == '查看详情'){
			str += '<div class="look_details" style="text-align:left;"><img src="../common/images/look_icon.jpg" style="width:20px; vertical-align:bottom;" alt="" /> <span>查看详情</span></div>';
		}else if(menu[x] == '商家备注'){
			str += '<div class="seller_note" style="text-align:left;"><img src="../common/images/modify_icon.png" style="width:17px; vertical-align:bottom;" alt="" /> <span>商家备注</span></div>';
		}else if(menu[x] == '关闭订单'){
			str += '<div class="close_order" style="text-align:left;"><img src="../common/images/close_icon.png" style="width:16px; vertical-align:sub;" alt="" /> <span>关闭订单</span></div>';
		}else{
